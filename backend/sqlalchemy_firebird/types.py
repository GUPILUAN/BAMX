...
class FBREAL(FBFLOAT):
    ...
    def __init__(self, precision=None, scale=None):
        super().__init__(precision=None, asdecimal=True)
...
class _FBFixedPoint(_FBNumeric):
    def __init__(
        ...
        super().__init__(precision, scale, decimal_return_scale, asdecimal=True)
...
class _FBLargeBinary(sqltypes.LargeBinary):
    ...
    def __init__(self, subtype=None, segment_size=None, charset=None, collation=None):
        super().__init__()
...
class _FBInterval(sqltypes.Interval):
    ...
    def bind_processor(self, dialect: Dialect):
        ...
        if impl_processor:
            ...
            def process_with_impl(value: Optional[dt.timedelta]):
                dt_value = value.total_seconds() / 86400 if value is not None else None
                return fixed_impl_processor(dt_value)

            return process_with_impl

        else:

            def process_without_impl(value: Optional[dt.timedelta]):
                return value.total_seconds() / 86400 if value is not None else None

            return process_without_impl
