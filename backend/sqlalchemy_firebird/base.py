...
class FBCompiler(sql.compiler.SQLCompiler):
    ...
    def function_argspec(self, func, **kw):
        if func.clauses is not None and len(func.clauses) > 0:
    ...
    def returning_clause(self, stmt, returning_cols, **kw):
        if getattr(self.dialect, "using_sqlalchemy2", False):
            return super().returning_clause(stmt, returning_cols, **kw)
        # For SQLAlchemy 1.4 compatibility only. Unneeded in 2.0.
        columns = [
            self._label_returning_column(stmt, c, populate_result_map=False)
...
class FBDDLCompiler(sql.compiler.DDLCompiler):
    ...
    def visit_create_index(
        ...
        first_expression = (
            index.expressions[0] if len(index.expressions) > 0 else index.expressions
        )
    ...
    def post_create_table(self, table):  # type: ignore
        table_opts = []
    ...
    def get_identity_options(self, identity_options):
        firebird_3_or_lower = (
            self.dialect.server_version_info and self.dialect.server_version_info < (4,)
        )
    ...
    def visit_identity_column(self, identity, **kw):
        ...
        kind = (
            "ALWAYS" if identity.always and (not firebird_3_or_lower) else "BY DEFAULT"
        )
...
class FBTypeCompiler(compiler.GenericTypeCompiler):
    def visit_boolean(self, type_, **kw):
        if self.dialect.server_version_info and self.dialect.server_version_info < (3,):
            # Cast Boolean type to SmallInteger for compatibility
            return self.visit_SMALLINT(sa_types.SmallInteger(), **kw)

        return self.visit_BOOLEAN(type_, **kw)

    def visit_datetime(self, type_, **kw):
        # Ensure type_ is of the expected TIMESTAMP type
        if not isinstance(type_, sa_types.TIMESTAMP):
            # Convert to TIMESTAMP if necessary
            type_ = sa_types.TIMESTAMP()
    ...
    def _render_string_type(
        ...
        charset = getattr(name, "charset", None)
    ...
    def visit_BINARY(self, type_: sa_types.BINARY, **kw) -> str:
        return super().visit_BINARY(type_, **kw)

    def visit_VARBINARY(self, type_: sa_types.VARBINARY, **kw) -> str:
    ...
    def visit_DECFLOAT(self, type_, **kw):
        return "DECFLOAT" + (type_.precision and "(%d)" % type_.precision or "")
    ...
    def visit_TIMESTAMP(self, type_, **kw):
        ...
        if self.dialect.server_version_info and self.dialect.server_version_info < (4,):
            return super().visit_TIMESTAMP(type_, **kw)

        return "TIMESTAMP%s %s" % (
            (
                "(%d)" % getattr(type_, "precision", 0)
                if getattr(type_, "precision", None) is not None
                else ""
            ),
            (getattr(type_, "timezone", False) and "WITH" or "WITHOUT") + " TIME ZONE",
        )
    ...
    def get_domains(self, schema: Optional[str] = None) -> List[ReflectedDomain]:
        with self._operation_context() as conn:
            ...
            return cast(
                ...
                self.dialect._load_domains(conn, schema, info_cache=self.info_cache),
            )
...
class FBDialect(default.DefaultDialect):
    ...
    type_compiler = (
        FBTypeCompiler  # For SQLAlchemy 1.4 compatibility only. Unneeded in 2.0.
    )
    ...
    def initialize(self, connection):
        ...
        if self.server_version_info and self.server_version_info < (3,):
            # Firebird 2.5
        ...
        elif self.server_version_info and self.server_version_info < (4,):
            # Firebird 3.0
    ...
    def get_table_names(self, connection, schema=None, **kw) -> List[str]:
        ...
        # Ensure only str is returned, filter out None and quoted_name
        names = [
    ...
    def get_temp_table_names(
        self, connection, schema: Optional[str] = None, **kw
    ) -> List[str]:
        temp_tables_query = """
            SELECT TRIM(rdb$relation_name) AS relation_name
            FROM rdb$relations
    ...
    def get_view_names(self, connection, schema=None, **kw):
        ...
        """
        return [
            str(self.normalize_name(row.relation_name))
    ...
    def get_sequence_names(self, connection, schema=None, **kw):
        ...
        return [
            str(self.normalize_name(row.generator_name))
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):  # noqa: C901
        ...
        is_firebird_25 = self.server_version_info and self.server_version_info < (3,)
    ...
    @reflection.cache
    def get_pk_constraint(self, connection, table_name, schema=None, **kw):  # type: ignore
        pk_query = """
        ...
        rows = c.fetchall()
        pkfields = [self.normalize_name(r.fname) for r in rows] if rows else None
        if pkfields:
            return {
    ...
    def get_foreign_keys(  # type: ignore
        self, connection, table_name: str, schema: Optional[str] = None, **kw
    ):
        fk_query = """
        ...
        tablename = self.denormalize_name(table_name)
        result_proxy = connection.exec_driver_sql(fk_query, (tablename,))

        fks: dict[str, dict[str, Any]] = defaultdict(
            lambda: {
                "name": None,
                "constrained_columns": [],
        ...
        return (
            []
            if not getattr(self, "using_sqlalchemy2", False)
            else reflection.ReflectionDefaults.foreign_keys()
        )

    @reflection.cache
    def get_indexes(self, connection, table_name, schema=None, **kw):  # type: ignore
        condition_source_expr = "TRIM(SUBSTRING(ix.rdb$condition_source FROM 6 FOR CHAR_LENGTH(ix.rdb$condition_source) - 5))"

        if self.server_version_info and self.server_version_info < (5,):
        ...
        def _adjust_column_names_for_expressions(result, tablename):
            ...
            for i in result:
                ...
                if expr is not None:
                    i["column_names"] = [
                        x if self.normalize_name(x) in colset else None for x in expr
                    ]
            return result
    ...
    @reflection.cache
    def get_unique_constraints(self, connection, table_name, schema=None, **kw):  # type: ignore
        ...
        ucs: dict[str, dict[str, Any]] = defaultdict(
            lambda: {"name": None, "column_names": []}
        )
    ...
    @reflection.cache
    def get_table_comment(self, connection, table_name, schema=None, **kw):  # type: ignore
        table_comment_query = """
    ...
    @reflection.cache
    def get_check_constraints(self, connection, table_name, schema=None, **kw):  # type: ignore
    ...
    def is_disconnect(self, e, connection, cursor):
        ...
        if self.dbapi and isinstance(e, (self.dbapi.DatabaseError)):
